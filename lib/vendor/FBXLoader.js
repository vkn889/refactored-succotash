...
class BinaryParser {
	...
	parse( buffer ) {
		...
		// PATCHED (see lib/vendor/README.md): some exporters' binary FBX
		// footer padding doesn't match the exact 160+16-byte/16-byte-aligned
		// shape endOfContent() expects, so the very last parseNode() call
		// reads a few stray footer bytes as a bogus property type and
		// throws — after the ENTIRE real scene (every node in Objects/
		// Connections/etc.) has already parsed successfully. Rather than
		// aborting the whole load over footer padding, stop gracefully and
		// keep whatever was already parsed once this happens within the
		// last ~1KB of the file (a genuine mid-file corruption will still
		// throw, since it won't be near the end).
		while ( ! this.endOfContent( reader ) ) {
			...
			try {
				...
				const remaining = reader.size() - reader.getOffset();
				if ( remaining > 1024 ) throw err;
				break;
